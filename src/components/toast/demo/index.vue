<template>
  <div class="demo toast">
    ...
    <demo-title :title="t('name')">
      <byted-icon-module-info theme="outline" size="20" fill="#333" />
    </demo-title>
...
<script>
import { BytedIconModuleInfo } from '@byte-design/vue-icons';
...
export default {
  i18n: {
    ...
    'en-US': {
      ...
      text: 'Text',
      words: 'words',
      manyWords: 'Many Words',
      success: 'Success',
      error: 'Error',
      warning: 'Warning',
      loading: 'loading',
      setTime: 'SetTime',
      setTimeText: 'Disappear after 1 second',
      forbidden: 'Forbidden Click Background',
      forbiddenText: 'Clicking any other button on the page is invalid',
      show: 'show',
      event: 'open & close',
      loadingDefault: 'Default Style',
      loadingCustom: 'Custom Words',
    },
  },
  components: {
    demoCell,
  ...
  methods: {
    ...
    clickLong() {
      this.$toast('If you have better suggestions. Please call totoo');
    },
    clickDuration() {
      this.$toast({
        message: 'ByDesign',
    ...
    clickForbid() {
      this.$toast({
        message: this.t('forbiddenText'),
        forbidClick: true,
      });
    },
    clickOpen() {
      this.$toast({
        message: 'Pay attention to console log',
        onOpened: () => {
          console.log('toast opened');
        },
        onClose: () => {
          console.log('toast colsed');
        },
      });
    },
  },
